	...
	public double solve(int LLHhIndex, ArrayList<Integer> heuristic_list, String fileName) {
		...
		for (int i = 0; i <= heuristic_list.size(); i++)
		{
			...
			switch (alg) {
				...
				for (int j = 0; j < REPEATE; j++) currentBin = NextFitDecreasing.startPacking(elements, bins, currentBin, rf.binSize, solel, solbin);
				break;
				case 10:  for (int j = 0; j < REPEATE; j++) currentBin = LargestFitDecreasing.startPacking(elements, bins, rf.binSize, solel, solbin);
				for (int j = 0; j < REPEATE; j++) currentBin = DjangAndFinch.startPacking(elements, bins, currentBin, rf.binSize, solel, solbin);
				break;
				case 11:  for (int j = 0; j < REPEATE; j++) currentBin = LargestFitDecreasing.startPacking(elements, bins, rf.binSize, solel, solbin);
				for (int j = 0; j < REPEATE; j++) currentBin = DjangAndFinchTuples.startPacking(elements, bins, currentBin, rf.binSize, solel, solbin);
				break;
				case 12:  for (int j = 0; j < REPEATE; j++) currentBin = LargestFitDecreasing.startPacking(elements, bins, rf.binSize, solel, solbin);
				for (int j = 0; j < REPEATE; j++) currentBin = LargestFitDecreasingFilter.startPacking(elements, bins, currentBin, rf.binSize, solel, solbin);
				break;
				case 13:  for (int j = 0; j < REPEATE; j++) currentBin = LargestFitDecreasing.startPacking(elements, bins, rf.binSize, solel, solbin);
				for (int j = 0; j < REPEATE; j++) currentBin = NextFitDecreasingFilter.startPacking(elements, bins, currentBin, rf.binSize, solel, solbin);
				break;
				case 14:  for (int j = 0; j < REPEATE; j++) currentBin = LargestFitDecreasing.startPacking(elements, bins, rf.binSize, solel, solbin);
				for (int j = 0; j < REPEATE; j++) currentBin = DjangAndFinchFilter.startPacking(elements, bins, currentBin, rf.binSize, solel, solbin);
				break;
				case 15:  for (int j = 0; j < REPEATE; j++) currentBin = LargestFitDecreasing.startPacking(elements, bins, rf.binSize, solel, solbin);
				for (int j = 0; j < REPEATE; j++) currentBin = DjangAndFinchTuplesFilter.startPacking(elements, bins, currentBin, rf.binSize, solel, solbin);
				break;
